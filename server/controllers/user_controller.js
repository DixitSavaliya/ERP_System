                        ...
                        if (rows.length > 0) {
                            bcrypt.compare(user.password, rows[0].password, function (error, doesMatch) {
                                if (doesMatch) {
                                    ...
                                    var user1 = {
                                        "id": rows[0].id,
                                        "secret_key": rows[0].secret_key ? rows[0].secret_key : '',
                                        "username": rows[0].username ? rows[0].username : '',
                                        "user_email": rows[0].email_id ? rows[0].email_id : '',
                                        "access_token": rows[0].access_token ? rows[0].access_token : '',
                                        "refresh_token": rows[0].refresh_token ? rows[0].refresh_token : '',
                                        "expire_time": rows[0].expire_time ? rows[0].expire_time : '',
                                        "server_time": dates._generateDateFormat("", "yyyy-mm-dd hh:ii:ss"),
                ...
                con1.query(sql, function (err, rows) {
                    ...
                    if (!err) {
                        if (rows[0] != undefined && rows[0].id != undefined && rows[0].id > 0) {
                            ...
                            console.log("user.secret_key --------------------> " + user.secret_key)
                            if (rows[0].email_id === user.email_id) {
...
function forgotPassword(req, res) {
    ...
    if (username != undefined && username != '' && username != null) {
        var sql = ` select 
                    ...
                    um.user_group 
                from user_master as um
                where um.username = '` + username + `' or um.email_id = '` + username + `'`;
        pool.getConnection(function (err1, con1) {
            if (!err1) {
                con1.query(sql, function (err, rows) {
                    if (err) {
...
function getUserBySearch(req, res) {
    ...
    var sql = ` select 
                ...
                from user_master as um
                where (um.user_group = 'freelancer' or um.user_group = 'company')
                `+ whereCond1 + `
